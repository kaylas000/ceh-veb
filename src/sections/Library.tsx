...
import { REFERENCES, SKILLS, PAIRS, TEXTURES } from "../data/library";
import { Reveal, useInView } from "../lib/fx";
import { Marquee, SectionHead } from "./Chrome";

const TABS = [
  { id: "refs", label: "Референсы", code: "REF", count: REFERENCES.length },
  { id: "skills", label: "Скилы", code: "SK", count: SKILLS.length },
  { id: "fonts", label: "Шрифты", code: "PAIRS", count: PAIRS.length },
  { id: "textures", label: "Текстуры", code: "T", count: TEXTURES.length },
] as const;

type TabId = (typeof TABS)[number]["id"];

function RefCard({ r, i }: { r: (typeof REFERENCES)[number]; i: number }) {
  const mined = r.id === "REF-07";
  return (
        ...
        <div className="grid lg:grid-cols-[1.15fr_1fr]">
          <div className="relative border-b-2 border-ink p-5 lg:border-b-0 lg:border-r-2">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className={`font-mono text-[10px] uppercase tracking-[0.22em] ${mined ? "text-green" : "text-red"}`}>
                  {r.id} · {r.style.join(" / ")}{mined ? " · добыт из боя" : ""}
                </p>
                <h3 className="mt-1 font-display text-2xl uppercase leading-none text-ink sm:text-3xl">{r.site}</h3>
              </div>
              <div className="hatch-pending relative hidden h-16 w-24 shrink-0 border border-ink/40 bg-paper-2 sm:block">
                <span className="absolute inset-0 grid place-items-center px-1 text-center font-mono text-[8px] uppercase leading-tight tracking-wider text-ink/55">
                  скрин: ждёт куратора
                </span>
              </div>
            </div>
          ...
          <div className="flex flex-col justify-between gap-4 bg-ink p-5 text-paper">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <a
                href={r.url.startsWith("http") ? r.url : "#proekty"}
...
function SkillCard({ s, i, open, onToggle }: { s: (typeof SKILLS)[number]; i: number; open: boolean; onToggle: () => void }) {
  return (
    <Reveal delay={Math.min(i * 70, 280)}>
    ...
    <section id="arhiv" className="relative bg-paper">
      ...
      <div className="relative mx-auto max-w-[1400px] px-4 py-16 sm:px-6 lg:py-24">
        <div ref={headRef} className={headIn ? "rv-in" : ""}>
          <SectionHead
            num="01"
            kicker="склад материалов"
            lines={[<>Архив — единственный</>, <span key="a" className="text-ink/50">источник</span>]}
            aside={
              <Reveal delay={150}>
        ...
        <div className="mt-10 flex flex-wrap border-2 border-ink" role="tablist" aria-label="Разделы архива">
          {TABS.map((t) => (
            <button
              key={t.id}
              role="tab"
            ...
            >
              ...
              {t.label}
              <span className={`border px-1.5 text-[10px] ${tab === t.id ? "border-paper/40" : "border-ink/30"}`}>{t.count}</span>
            </button>
          ))}
        </div>
        ...
        <div className="mt-6">
          ...
          {tab === "skills" && (
            <div className="space-y-4">
              ...
              {SKILLS.map((s, i) => (
                <SkillCard key={s.id} s={s} i={i} open={openSkill === i} onToggle={() => setOpenSkill(openSkill === i ? -1 : i)} />
              ))}
            </div>
          )}
              ...
              <div className="border-2 border-ink bg-card">
                {PAIRS.map((p, i) => (
                  <Reveal key={p.display} delay={Math.min(i * 60, 240)}>
                    <div className={`grid gap-4 px-5 py-5 lg:grid-cols-[1.4fr_1fr_1.2fr] ${i > 0 ? "border-t-2 border-ink" : ""}`}>
                      <div>
                        <p className="text-3xl leading-tight text-ink" style={{ fontFamily: `"${p.display}", "Russo One", sans-serif` }}>
                          {p.display}
                        </p>
                      ...
                      <div className="flex items-start">
                        <span className="bg-ink px-2 py-1 font-mono text-[10px] uppercase tracking-[0.2em] text-paper">{p.role}</span>
                      </div>
            ...
            <div className="grid gap-5 md:grid-cols-3">
              {TEXTURES.map((t, i) => (
                <Reveal key={t.id} delay={i * 90}>
                  <article className="border-2 border-ink bg-card">
                    <div className="relative h-32 overflow-hidden border-b-2 border-ink bg-ink">
                      <span className="absolute left-3 top-3 bg-red px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.2em] text-paper">
                        {t.id}
                      </span>
                      <span className="absolute bottom-2 right-3 font-mono text-[9px] uppercase tracking-wider text-paper/50">
      ...
      <Marquee
        items={[
          ...
          "приём без источника — слоп (К-04)",
          "каждая запись: техники ≥3 + takeaway",
          "скриншоты дольёт куратор",
          "поиск по техникам: grep",
        ]}
      />
    </section>
  );
