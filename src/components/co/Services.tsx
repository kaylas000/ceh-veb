...
import { COMPANY, SERVICES, type RalColor } from "../../data/company";
...
function Calculator({ selected }: { selected: RalColor }) {
  ...
  const custom = svc.base === null;

  const total = useMemo(() => {
    if (custom || !svc.base) return null;
    const q = Math.max(1, qty);
    let v = svc.base * q;
    if (metallic) v *= 1.15;
    if (svc.unit === "m2" && q > 20) v *= 0.9;
    return Math.round(v / 10) * 10;
  }, [custom, svc, qty, metallic]);

  const [resRef, resIn] = useInView<HTMLDivElement>(0.3);
  const shown = useCountUp(total ?? 0, resIn && total !== null, 900);

  const unitLabel = svc.unit === "pc" ? "шт" : "м²";
  ...
  return (
    <div ref={resRef} className="border-2 border-steel-2 bg-coal-2 shadow-[10px_10px_0_rgba(0,0,0,0.35)]">
      ...
      <div className="space-y-4 p-5">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-fog-2">позиция прайса</p>
        ...
        {!custom && (
          <div className="grid grid-cols-[1fr_auto] items-end gap-3">
            ...
            <div className="flex gap-1.5 pb-1">
              {svc.unit === "m2" && [12, 24, 46].map((q) => (
                <button key={q} onClick={() => setQty(q)} className="border border-steel px-2 py-1 font-mono text-[10px] text-fog transition-colors hover:border-heat hover:text-heat">
                  {q}
                </button>
              ))}
              {svc.unit === "pc" && [1, 2, 4].map((q) => (
                <button key={q} onClick={() => setQty(q)} className="border border-steel px-2 py-1 font-mono text-[10px] text-fog transition-colors hover:border-heat hover:text-heat">
                  {q}
                </button>
              ))}
        ...
        <div className="border-t-2 border-steel pt-4">
          {custom ? (
            <>
              <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-fog-2">предварительно</p>
              <p className="mt-1 font-display text-3xl font-black uppercase text-concrete">по запросу</p>
              <p className="mt-1 text-[12px] leading-snug text-fog">
                Пришлём смету по фото детали в течение часа в рабочее время.
              </p>
            </>
          ) : (
            <>
              ...
              <p className="mt-2 font-mono text-[10px] leading-relaxed text-fog-2">
                ...
                {svc.unit === "m2" && qty > 20 ? " × 0.9 (партия)" : ""}
                {svc.unit === "m2" && qty <= 20 ? " · от 20 м² — скидка 10%" : ""}
              </p>
            </>
          )}
          <a
      ...
      <div className="relative mx-auto max-w-[1400px] px-4 py-20 sm:px-6 lg:py-24">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <Reveal delay={150}>
            <p className="max-w-sm text-[14px] leading-relaxed text-fog">
              Шесть позиций без мелкого шрифта. Точную смету считаем по чертежу или фото —
              по телефону <a href={COMPANY.phoneHref} className="text-concrete underline decoration-heat underline-offset-4">{COMPANY.phone}</a>.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[1.4fr_1fr] lg:gap-14">
          ...
          <div className="border-2 border-steel">
            {SERVICES.map((s, i) => (
              <Reveal key={s.id} delay={Math.min(i * 60, 300)}>
                <div
                  className={`group grid gap-3 px-5 py-5 transition-colors duration-200 hover:bg-coal-3 sm:grid-cols-[52px_1fr_auto] sm:items-center sm:gap-5 ${
                    i > 0 ? "border-t-2 border-steel" : ""
                  }`}
                >
                  <span className="font-mono text-xs text-heat">{String(i + 1).padStart(2, "0")}</span>
                  <div>
                    <h3 className="font-display text-lg font-bold uppercase leading-tight transition-colors duration-200 group-hover:text-heat sm:text-xl">
                      {s.title}
                    </h3>
                    <p className="mt-1 font-mono text-[11px] uppercase tracking-[0.14em] text-fog-2">{s.note}</p>
                  </div>
                  <div className="text-left sm:text-right">
                    <p className="font-display text-2xl font-black leading-none text-concrete transition-colors duration-200 group-hover:text-heat">
                      {s.price}
                    </p>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 px-5 py-3">
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-fog-2">пескоструй и подготовка — включены в цену</span>
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-amber">партии от 20 м² — скидка 10%</span>
            </div>
          </div>
