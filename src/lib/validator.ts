import type { FS } from "../data/fixtures";
...
function read(fs: FS, path: string): string | undefined {
  return fs[path];
}
...
function pairFamilies(fs: FS): Set<string> {
  const fams = new Set<string>();
  const pairs = read(fs, "assets/fonts/PAIRS.md") ?? "";
...
export function lintSlop(fs: FS, root: string): Violation[] {
  ...
  if (families.size === 1) {
    ...
    out.push({
      ...
      file: "site/*",
      line: 0,
      snippet: `единственный шрифт «${only}» без пары из PAIRS.md${allowed.has(only) ? "" : " (вне пар)"}`,
    });
  }
  return out;
}
...
export function validate(fs: FS, root: string): Report {
  const rows: CheckRow[] = [];
  const read2 = (p: string) => read(fs, `${root}/${p}`);

  const direction = read2("DIRECTION.md");
  const sources = read2("SOURCES.md");
  const structure = read2("STRUCTURE.md");
  const seed = read2("SEED.md");
  const review = read2("REVIEW.md");
  ...
  /* V-01 */
  {
    ...
    const ok = !direction ? false : refs.length >= 3 && takeaways.length >= 3 && anti;
    rows.push({
      code: "V-01",
  ...
  /* V-06 — easing */
  {
    const curvesRaw = read(fs, "motion/easing-curves.json") ?? "{}";
    let curves: Record<string, string> = {};
    try {
      curves = JSON.parse(curvesRaw) as Record<string, string>;
    } catch {
      curves = {};
    }
  ...
  /* V-09 — diff-projects */
  {
    ...
    if (!structure) {
      ...
      rows.push({
        ...
        detail: `максимальное сходство: ${pct}% (${worstP || "история пуста"}); секций: ${sec.length}, пороги не превышены`,
  ...
  /* V-10 — полнота meta.yaml */
  {
    ...
    for (const id of cited) {
      ...
      if (problems.length) {
        ...
        ev.push(`${id}: schema валидна (techniques ${techs}, palette ${pals})`);
      }
    }
    rows.push({
      code: "V-10",
